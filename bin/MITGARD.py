...
def _SingleEnd_(sample,single,reference,N,M,lowcoverage,gc,clade):
    #index reference
...
def _PairedEnd_(sample,pairONE,pairTWO,reference,N,M,lowcoverage,gc,clade):
    #index reference
    print("\n"+dt.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+" >>>> indexing reference...\n")
    os.system("mkdir bowtie_index")
    os.system("bowtie2-build "+reference+" bowtie_index/REF")
...
def __main__():
    ...
    #add an option to perform rearrangement check!!! using the RearrangementCheck.py
