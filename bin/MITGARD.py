...
try:
    ...
    print("Biopython module is not installed.\nInstall it using \"pip install biopython\" or check https://biopython.org")
...
def _ParseFasta_(out, fasta1, fasta2):
    ...
    for record in SeqIO.parse(fasta2, "fastq"):
        SEQ = str(record.seq)
        PHRED = "G"*len(SEQ)
        count += 1
        OUT.write(">seq"+str(count)+"\n"+SEQ+"\n")
...
def _PairedEnd_(sample,pairONE,pairTWO,reference,N,M,lowcoverage):
    ...
    _ParseFasta_("mapped/"+sample+"_ENDTOEND_mapped.fq", "mapped/"+sample+"_ENDTOEND_mapped_1.fasta", "mapped/"+sample+"_ENDTOEND_mapped_1.fasta")
    ##FASTQ
